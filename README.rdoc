...
[URL]            <code>https://rpm.newrelic.com/accounts/:account_id/applications.xml</code>  Replace <tt>:account_id</tt> with the account ID.
...
Unlike other API requests, the deployments API will accept either a license key or an API key in the +api-key+ header.  Furthermore, when 
using the license key, you do not need to enable the API in the account settings.  This is the standard mechanism used by the agent to upload
deployments from the built in capistrano recipes.
...
Here's an example using <tt>curl</tt>:
    ...
    curl -H "x-api-key:YOUR_LICENSE_KEY_HERE" -d "deployment[application_id]=iMyFace.ly Production" https://rpm.newrelic.com/deployments.xml
...
If you were to specify the optional <tt>host</tt>, <tt>description</tt>, <tt>changelog</tt> and <tt>user</tt> fields, the command would look like this:
    ...
    curl -H "x-api-key:YOUR_LICENSE_KEY_HERE" -d "deployment[application_id]=iMyFace.ly Production" -d "deployment[host]=localhost" -d "deployment[description]=This deployment was sent using curl" -d "deployment[changelog]=many hands make light work" -d "deployment[user]=Joe User" https://rpm.newrelic.com/deployments.xml
