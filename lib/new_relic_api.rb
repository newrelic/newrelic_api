...
module NewRelic
  class << self
    def create_api_instance(name='')    
      @module_name = 'NewRelicApi' + name.capitalize
      class_eval <<-"end_eval",__FILE__,__LINE__
        module ::#{@module_name}
          ...
          class BaseResource < ActiveResource::Base #:nodoc:
            ...
            class << self
              ...
              def inherited(klass) #:nodoc:
                #{@module_name}.track_resource(klass)
              end

              def headers
                raise "api_key required" unless #{@module_name}.api_key
                {'x-api-key' => #{@module_name}.api_key}
              end

              def site_url
                host = #{@module_name}.host || 'rpm.newrelic.com'
                port = #{@module_name}.port || 80
          ...
          # Find Accounts
          #
          #   #{@module_name}::Account.find(:first) # find account associated with the api key
          ...
          # ==Example
          #
          #   #{@module_name}::Deployment.create :application_id => 11142007, :description => "Update production", :user => "Big Mike"
          #
          #   #{@module_name}::Deployment.create :app_name => "My Application", :description => "Update production", :user => "Big Mike"
          #
...
# default instance (for backwards compatibliity)
::NewRelicApi = NewRelic::create_api_instance('')